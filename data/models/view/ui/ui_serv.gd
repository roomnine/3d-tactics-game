...
func set_actions_menu_visibility(v: bool, unit: DefaultUnit, control: TacticsControls) -> void:
	if not control.get_node("ControlArea").visible:
		control.get_node("ControlArea/Actions/Move").grab_focus() # Focus on Move action if menu isn't visible
	...
	control.get_node("ControlArea/Actions/Move").disabled = not unit.res.can_move
	control.get_node("ControlArea/Actions/Attack").disabled = not unit.res.can_attack
